...
public class InterviewService {
    ...
    // Allow Jitsi Meet domains, common meeting platforms, and local time-gated pages
    private static final Pattern MEETING_URL_PATTERN = Pattern.compile(
        "^https?://(meet\\.jit\\.si|zoom\\.us|teams\\.microsoft\\.com|meet\\.google\\.com)/.*",
        Pattern.CASE_INSENSITIVE
    );

    // Local time-gated meeting page generated by MeetingService
    private static final Pattern LOCAL_MEETING_PATTERN = Pattern.compile(
        "^https://meet\\.jit\\.si/TalentBridge-Interview-.+",
        Pattern.CASE_INSENSITIVE
    );
    ...
    private static boolean isValidUrl(String url) {
        ...
        if (LOCAL_MEETING_PATTERN.matcher(url).matches()) return true;
        if (MEETING_URL_PATTERN.matcher(url).matches()) return true;

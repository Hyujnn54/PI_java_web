...
public class MatchingService {
    ...
    private double calculateSkillLevelScore(SkillLevel candidateLevel, SkillLevel requiredLevel) {
        ...
        if (candidateValue >= requiredValue) {
            return 100; // Niveau égal ou supérieur
        } else if (candidateValue == requiredValue - 1) {
            return 70;  // Un niveau en dessous
        } else {
            return 40;  // Deux niveaux ou plus en dessous
        }
    }
     ...
     */
    private double calculateContractTypeScore(List<ContractType> preferredTypes, ContractType offerType) {
        ...
        for (ContractType preferred : preferredTypes) {
     ...
     */
    private boolean areContractTypesCompatible(ContractType type1, ContractType type2) {
        // CDI et FULL_TIME sont compatibles
        if ((type1 == ContractType.CDI && type2 == ContractType.FULL_TIME) ||
            (type1 == ContractType.FULL_TIME && type2 == ContractType.CDI)) {
            return true;
        }
        // CDD et PART_TIME peuvent être compatibles
        if ((type1 == ContractType.CDD && type2 == ContractType.PART_TIME) ||
            (type1 == ContractType.PART_TIME && type2 == ContractType.CDD)) {
            return true;
        }
        return false;
    }

...
public class EventRegistrationService {
    ...
    public List<EventRegistration> getAll() throws SQLException {
        ...
        String query = "SELECT er.*, re.title, re.event_date, re.event_type, re.meet_link FROM event_registration er JOIN recruitment_event re ON er.event_id = re.id";
    ...
    public List<EventRegistration> getByCandidate(long candidateId) throws SQLException {
        checkConnection();
        List<EventRegistration> registrations = new ArrayList<>();
        String query = "SELECT er.*, u.first_name, u.last_name, u.email, re.title as event_title, re.event_date, re.event_type, re.location, re.description, re.meet_link FROM event_registration er " +
    ...
    public List<EventRegistration> getByEvent(long eventId) throws SQLException {
        ...
        String query = "SELECT er.*, u.first_name, u.last_name, u.email, re.title as event_title, re.event_date, re.event_type, re.meet_link FROM event_registration er " +
                "JOIN recruitment_event re ON er.event_id = re.id " +
                "LEFT JOIN candidate c ON er.candidate_id = c.id " +
                "LEFT JOIN users u ON (er.candidate_id = u.id OR c.user_id = u.id) " +
                "WHERE er.event_id = ?";
    ...
    private EventRegistration mapResultSetToRegistration(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            try {
                ...
                // event_type or meet_link not in result set
            }
            reg.setEvent(event);
        } catch (SQLException e) {
            // event_title or event_date not in result set
        }
    ...
    public int getConfirmedCount(long eventId) throws SQLException {
        checkConnection();
        // Count all statuses that indicate a valid registration
        String query = "SELECT COUNT(*) FROM event_registration WHERE event_id = ? AND attendance_status IN ('CONFIRMED', 'REGISTERED', 'ATTENDED')";
        PreparedStatement ps = connection.prepareStatement(query);
        ps.setLong(1, eventId);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
